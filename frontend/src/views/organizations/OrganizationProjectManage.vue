    ...
    <div class="container mx-auto px-4">
      <div class="bg-white rounded-xl shadow-md p-6">
        <div class="flex flex-col md:flex-row md:items-end md:justify-between gap-4 mb-6">
          <div>
            <div class="text-2xl font-bold text-dark mb-1">我发布的项目</div>
            <div class="text-gray-600 text-sm">公益组织维护自己的项目</div>
          </div>
          <div class="flex flex-col sm:flex-row gap-3">
            <el-input v-model="query.keyword" placeholder="搜索标题/摘要" clearable class="sm:w-72" @keyup.enter="load" />
            <el-select v-model="query.status" placeholder="状态" clearable class="sm:w-40" @change="load">
              <el-option label="草稿" value="DRAFT" />
              <el-option label="进行中" value="ACTIVE" />
              <el-option label="已完成" value="COMPLETED" />
            </el-select>
            <button class="btn-secondary px-5 py-2" @click="openCreate"><i class="fa-solid fa-plus mr-2"></i>新建</button>
